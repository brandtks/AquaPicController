...
namespace AquaPic.Gadgets.Sensor.FloatSwitch
{
    ...
    public class SwitchTypeMutator : ISettingMutator<SwitchType>
    {
        public SwitchType Read (JObject jobj, string[] keys) {
            if (keys.Length < 1) {
        ...
        public void Write (SwitchType value, JObject jobj, string[] keys) {
            if (keys.Length < 1) {
                throw new ArgumentException ("keys can not be empty", nameof (keys));
            }
        ...
        public bool Valid (SwitchType type) {
            return true;
        }

        public SwitchType Default () {
            return SwitchType.NormallyOpened;
        }
    }
}
